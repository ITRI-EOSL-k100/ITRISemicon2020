...
import com.itri.itrisemicon2020.fft.Complex
import com.itri.itrisemicon2020.fft.FFT
import kotlinx.android.synthetic.main.activity_main.*
import java.io.File
import java.util.*
import kotlin.concurrent.timer

class MainActivityFFT : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bleConnector.run {
            onConnecting = {
                ...
                progressDialog = ProgressDialog.show(this@MainActivityFFT, null, "連線中，請稍候")
            }
            onServiceDiscovering = {
                runOnUiThread {
                    ...
                    toggleRecord?.isEnabled = false
                    progressDialog?.dismiss()
                    progressDialog = ProgressDialog.show(this@MainActivityFFT, null, "正在搜尋服務 UUID")
                }
            }
            onServiceDiscoverCompleted = {
                runOnUiThread {
                    ...
                    progressDialog?.dismiss()
                    Toast.makeText(this@MainActivityFFT, "搜尋完成", Toast.LENGTH_SHORT).show()
                }
            }
            onDisconnected = {
                runOnUiThread {
...
//        }
    ...
    private fun initChart() {
        ...
        chartFFT?.run {
            ...
            data = LineData(generateLineDataSet(ArrayList(), "f"))
        }
    }
    ...
    private fun fft(dataList: List<Record>) {
        ...
        val P2 = ArrayList<Double>()    // P2 list
        for (i in Y.indices) {
            ...
            timer(null, true, 0, 1000) {
                if (bleConnector.selectedDevice() != null) {
                    ...
                    runOnUiThread {
                        ...
                        if (bleList.size == 0) {
                            dialog.dismiss()
                            Toast.makeText(this@MainActivityFFT, "未發現裝置", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            }
